/**
 * TraceWriter — deterministic per-request trace files for Trestle.
 *
 * Writes structured JSONL trace files to ~/.trestle/traces/{sessionId}/{YYYY-MM-DD}/{traceId}.jsonl
 * and maintains a SQLite index at ~/.trestle/traces/index.db.
 *
...
export function defaultTracesConfig(): TracesConfig {
  return {
    ...
    retentionDays: 30,
    directory: path.join(os.homedir(), '.trestle', 'traces'),
    maxDiskMb: 500,
  };
}
