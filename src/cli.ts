 ...
 *
 * Usage:
 *   npx @trestle/proxy [command] [options]
 *   trestle-proxy [command] [options]
 *
 * Commands:
 *   (default)              Start the proxy server
 *   status                 Show proxy status (circuit state, stats, process info)
 *   enable                 Enable Trestle proxy routing
 *   disable                Disable Trestle proxy routing (passthrough mode)
...
import { fireDashboardLinked } from "./observability/lifecycle-telemetry.js";
import {
  loadConfig,
  isFirstRun,
...
import { getRoutingLog } from "./observability/routing-log.js";
...
const CREDENTIALS_PATH = join(homedir(), ".trestle", "credentials.json");
...
const API_URL = process.env.TRESTLE_API_URL || "https://api.relayplane.com";
...
async function handleLoginCommand(): Promise<void> {
  ...
  console.log("  Use the local dashboard at http://localhost:4100");
  console.log("");
}
...
async function handleCloudStatusCommand(): Promise<void> {
  ...
  try {
    ...
    const res = await fetch("http://127.0.0.1:4100/health", {
      signal: controller.signal,
    });
    clearTimeout(timeout);
  ...
  console.log(`  Config:      ~/.trestle/config.json`);
...
async function handleEnsureRunning(): Promise<void> {
  ...
  const relayDir = join(homedir(), ".trestle");
  ...
  if (await isPortListening()) {
    console.log(`Trestle already running on :${PORT}`);
    return;
  }
  ...
  while (Date.now() < deadline) {
    ...
    if (await isPortListening()) {
      console.log(`Trestle started (pid: ${pid})`);
      return;
    }
  }

  process.stderr.write(
    `Error: Trestle did not start within 3s (pid: ${pid}, log: ${logFile})\n`,
  );
  process.exit(1);
}
...
function printHelp(): void {
  console.log(`
Trestle Proxy - Intelligent AI Model Routing
...
Commands:
  ...
  login                  Log in to Trestle (opens browser)
  ...
  stats                  Show usage statistics
  config                 Show configuration
  config set-key <key>   Set Trestle API key
...
Example:
  ...
  npx @trestle/proxy --audit

  # Start in offline mode (no telemetry transmission)
  npx @trestle/proxy --offline

  # Disable telemetry completely
  npx @trestle/proxy telemetry off
...
function printVersion(): void {
  console.log(`Trestle Proxy v${VERSION}`);
}
...
function handleTelemetryCommand(args: string[]): void {
  ...
  switch (subcommand) {
    case "on":
      ...
      console.log(
        "   Anonymous usage data will be collected to improve routing.",
      );
    ...
    default:
      ...
      console.log("");
      console.log("   To enable:  trestle-proxy telemetry on");
      console.log("   To disable: trestle-proxy telemetry off");
      console.log("   To audit:   trestle-proxy --audit");
      console.log("");
      break;
  }
}
...
function handleLifecycleCommand(args: string[]): void {
  ...
  switch (subcommand) {
    ...
    case "off":
      ...
      console.log(
        "   No pings will be sent. The proxy continues to work normally.",
      );
      break;
    ...
    default: {
      ...
      console.log(
        "   Sends: anonymous install + daily session + dashboard-link pings.",
      );
      console.log(
        "   Never sends: request content, model names, tokens, costs.",
      );
      console.log("");
      console.log("   To enable:  trestle lifecycle on");
      console.log("   To disable: trestle lifecycle off");
      console.log("");
      break;
    }
  }
...
function handleStatsCommand(): void {
  ...
  if (stats.savings > 0) {
    console.log(
      `  💰 You saved:   $${stats.savings.toFixed(4)} (${stats.savingsPercent.toFixed(1)}%)`,
    );
  } else if (stats.totalEvents > 0 && stats.baselineCost === 0) {
    console.log(
      `  ⚠️  No token data yet — savings will appear after new requests`,
    );
  }
    ...
    for (const [model, data] of Object.entries(stats.byModel)) {
      const savingsNote =
        data.baselineCost > 0
          ...
          : "";
      console.log(
        `    ${model}: ${data.count} requests, $${data.cost.toFixed(4)}${savingsNote}`,
      );
    }
    console.log("");
  }
  ...
  if (Object.keys(stats.byTaskType).length > 0) {
    ...
    for (const [taskType, data] of Object.entries(stats.byTaskType)) {
      console.log(
        `    ${taskType}: ${data.count} requests, $${data.cost.toFixed(4)}`,
      );
    }
    console.log("");
  }
...
async function handleStatusCommand(): Promise<void> {
  ...
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 2000);
    const res = await fetch(`${resolved.proxyUrl}/health`, {
      signal: controller.signal,
    });
    clearTimeout(timeout);
    proxyReachable = res.ok;
...
function handleEnableDisableCommand(enable: boolean): void {
  ...
  console.log(`✅ Trestle ${enable ? "enabled" : "disabled"}`);
...
function handleConfigCommand(args: string[]): void {
  ...
  console.log(
    `  Telemetry:   ${config.telemetry_enabled ? "✅ Enabled" : "❌ Disabled"}`,
  );
  console.log(
  ...
  console.log("");
  console.log("  To set API key: trestle-proxy config set-key <your-key>");
  console.log("");
}
...
const TRESTLE_CONFIG_PATH = join(homedir(), ".trestle", "config.json");
const SERVICE_NAME = "trestle-proxy";
const SERVICE_PATH = `/etc/systemd/system/${SERVICE_NAME}.service`;

function loadRelayplaneConfig(): Record<string, any> {
  try {
    if (existsSync(TRESTLE_CONFIG_PATH)) {
      return JSON.parse(readFileSync(TRESTLE_CONFIG_PATH, "utf8"));
    }
  } catch {}
  return {};
}
...
function saveRelayplaneConfig(config: Record<string, any>): void {
  const dir = dirname(TRESTLE_CONFIG_PATH);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
  writeFileSync(TRESTLE_CONFIG_PATH, JSON.stringify(config, null, 2) + "\n");
}
...
function sanitizePosixUsername(raw: string | undefined): string | undefined {
  ...
  if (!/^[a-zA-Z0-9_][a-zA-Z0-9_\-\.]{0,31}$/.test(cleaned)) {
    console.error(
      `SUDO_USER "${cleaned}" is not a valid POSIX username. Aborting.`,
    );
    process.exit(1);
  }
  return cleaned;
...
async function handleAutostartCommand(args: string[]): Promise<void> {
  ...
  if (sub === "on") {
    if (!isRoot()) {
      console.log(
        "  ⚠️  Autostart requires root. Try: sudo trestle autostart on",
      );
      return;
    }
    ...
    let binPath: string;
    try {
      binPath = execSync("which trestle", { encoding: "utf8" }).trim();
    } catch {
      if (isRoot()) {
        console.warn(
          "  ⚠️  Could not find relayplane in PATH (sudo may have stripped it).",
        );
        console.warn(
          '     If the service fails to start, try: sudo env "PATH=$PATH" trestle autostart on',
        );
      }
    ...
    if (sudoUser === "root") {
      ...
      serviceHome =
        userEnv && userEnv !== "root" ? `/home/${userEnv}` : homedir();
    }
    const resolvedAutoHome = resolve(serviceHome);
    if (
      resolvedAutoHome !== "/root" &&
      !resolvedAutoHome.startsWith("/home/")
    ) {
      console.error(
        `Computed home "${resolvedAutoHome}" is outside expected paths. Aborting.`,
      );
      process.exit(1);
    }
...
`;
    ...
    execSync(
      "systemctl daemon-reload && systemctl enable trestle-proxy && systemctl start trestle-proxy",
      { stdio: "inherit" },
    );
    ...
    console.log(
      "  ✅ Autostart enabled. Trestle will start on boot and restart on crash.",
    );
    return;
  }
  ...
  if (sub === "off") {
    if (!isRoot()) {
      console.log(
        "  ⚠️  Autostart requires root. Try: sudo trestle autostart off",
      );
      return;
    }
    ...
    try {
      execSync(
        "systemctl stop trestle-proxy && systemctl disable trestle-proxy",
        { stdio: "inherit" },
      );
    } catch {
  ...
  try {
    const active = execSync(`systemctl is-active ${SERVICE_NAME} 2>/dev/null`, {
      encoding: "utf8",
    }).trim();
  ...
  console.log("");
  if (!isEnabled) {
    console.log("  To enable:  sudo trestle autostart on");
  } else {
    console.log("  To disable: sudo trestle autostart off");
  }
  console.log("");
}
...
async function handleMeshCommand(args: string[]): Promise<void> {
  ...
  if (sub === "status") {
    ...
    try {
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 2000);
      const res = await fetch("http://127.0.0.1:4100/v1/mesh/stats", {
        signal: controller.signal,
      });
      clearTimeout(timeout);
      if (res.ok) {
  ...
  if (sub === "off") {
    ...
    console.log(
      "  ❌ Mesh disabled. Restart the proxy for changes to take effect.",
    );
    return;
  }
  ...
  if (sub === "sync") {
    try {
      const res = await fetch("http://127.0.0.1:4100/v1/mesh/sync", {
        method: "POST",
      });
      if (res.ok) {
        const data = (await res.json()) as {
          sync: { pushed?: number; pulled?: number; errors?: string[] };
        };
  ...
  if (sub === "contribute") {
    ...
    console.log("Usage: trestle mesh contribute [on|off|status]");
    return;
  }

  console.log(
    "Unknown mesh subcommand. Available: status, on, off, sync, contribute",
  );
}
...
function getServiceAssetPath(): string {
  return join(__dirname, "..", "assets", "trestle-proxy.service");
}
...
<dict>
  ...
  <string>${homedir()}/Library/Logs/trestle-proxy.log</string>
  <key>StandardErrorPath</key>
  <string>${homedir()}/Library/Logs/trestle-proxy.error.log</string>
...
async function handleServiceCommand(args: string[]): Promise<void> {
  ...
  if (!isMac && !isLinux) {
    console.log(
      "  ⚠️  Service management is only supported on Linux (systemd) and macOS (launchd).",
    );
    return;
  }
  ...
  try {
    ...
    if (isRoot()) {
      ...
      console.warn(
        '     If the service fails to start, try: sudo env "PATH=$PATH" relayplane service install',
      );
    }
    binPath = process.argv[0] ?? "relayplane";
  }
  ...
  if (sub === "install") {
    if (isLinux) {
      ...
      if (!isRoot() && !dryRun) {
        console.log(
          "  ⚠️  Service install requires root. Try: sudo relayplane service install",
        );
        return;
      }
      ...
      if (
        resolvedSvcHome !== "/root" &&
        !resolvedSvcHome.startsWith("/home/")
      ) {
        console.error(
          `Computed home "${resolvedSvcHome}" is outside expected paths. Aborting.`,
        );
        process.exit(1);
      }
...
`;
      ...
      for (const key of envKeys) {
        if (
          process.env[key] &&
          !serviceContent.includes(`Environment=${key}=`)
        ) {
          serviceContent = serviceContent.replace(
            /\[Install\]/,
      ...
      if (dryRun) {
        console.log("");
        console.log(
          "  [DRY RUN] Would write to /etc/systemd/system/trestle-proxy.service:",
        );
        console.log("  ─────────────────────────────────────────");
        console.log(serviceContent);
        console.log("  ─────────────────────────────────────────");
        console.log(
          "  [DRY RUN] Would run: systemctl daemon-reload && systemctl enable --now trestle-proxy",
        );
        console.log("");
        return;
      }
      ...
      chmodSync(SERVICE_PATH, 0o600);
      execSync(
        "systemctl daemon-reload && systemctl enable --now trestle-proxy",
        { stdio: "inherit" },
      );
      ...
      console.log("     Trestle will start on boot and restart on crash.");
      console.log("     Run `trestle service status` to verify.");
      console.log("");
    } else if (isMac) {
      ...
      console.log("     Trestle will start on login and restart on crash.");
      console.log("");
    }
    return;
  }
  ...
  if (sub === "uninstall") {
    if (isLinux) {
      if (!isRoot() && !dryRun) {
        console.log(
          "  ⚠️  Service uninstall requires root. Try: sudo relayplane service uninstall",
        );
        return;
      }
      ...
      if (dryRun) {
        console.log("");
        console.log(
          "  [DRY RUN] Would run: systemctl stop trestle-proxy && systemctl disable trestle-proxy",
        );
        console.log(
          "  [DRY RUN] Would remove /etc/systemd/system/trestle-proxy.service",
        );
  ...
  if (isLinux && hasSystemd()) {
    ...
    try {
      statusOutput = execSync(`systemctl status ${SERVICE_NAME} 2>&1 || true`, {
        encoding: "utf8",
      });
    } catch {}
    ...
    try {
      const output = execSync("launchctl list com.relayplane.proxy 2>&1", {
        encoding: "utf8",
      });
...
async function main(): Promise<void> {
  ...
  if (command === "start") {
    // "trestle start" just falls through to start the server
    args.shift();
  }
  ...
  if (
    ...
    console.log(
      "     Claude Code (Max plan) users: this is correct, no key needed.",
    );
    console.log(
      "     API key users: set ANTHROPIC_API_KEY to enable env-based auth.",
    );
    console.log("");
  }
  ...
  if (offline) {
    ...
    console.log(
      "    📊 Telemetry enabled (--audit to inspect, telemetry off to disable)",
    );
  } else {
    console.log("    📴 Telemetry disabled");
  }
  ...
  if (creds?.apiKey && !offline) {
    ...
    console.log("    💻 Local only (run `trestle login` for cloud sync)");
  }
  ...
  try {
    ...
    if (!hasAnthropicKey) {
      console.log(
        "  Tip: add the export to your ~/.zshrc or ~/.bashrc to make it permanent.",
      );
      console.log("");
    }
 ...
 * Prompts for: API key, daily budget cap, routing mode → writes ~/.trestle/config.json.
 *
...
async function handleInitWizard(): Promise<void> {
  const configDir = join(homedir(), ".trestle");
  ...
  if (!isTTY) {
    ...
    if (hasOpenRouterKey && !hasAnthropicKey && !hasOpenAIKey) {
      try {
        ...
        if (!rawCfg["defaultProvider"]) {
          ...
          console.log(
            '[Trestle] Auto-configured defaultProvider: "openrouter" (OpenRouter-only setup detected)',
          );
        }
      } catch {
    ...
    console.log("✅ Trestle initialized");
  ...
  const existingProviders = rawConfig["providers"] as
  ...
  const existingBudget = rawConfig["budget"] as
    | Record<string, unknown>
    | undefined;
  const existingDailyUsd =
    typeof existingBudget?.["dailyUsd"] === "number"
      ? (existingBudget["dailyUsd"] as number)
      : null;
  const existingOnBreach =
    typeof existingBudget?.["onBreach"] === "string"
  ...
  console.log("");
  console.log("  ╭─────────────────────────────────────────────╮");
  console.log("  │       Trestle Setup Wizard               │");
  ...
  console.log(hr);
  console.log("  Trestle routes requests through your own API key.");
  console.log("  Your key is stored locally in ~/.trestle/config.json");
  console.log("  and is never sent to Trestle servers.");
  console.log("");

  let apiKeyDisplay = "";
  if (existingApiKey) {
    const masked =
      existingApiKey.slice(0, 12) + "****" + existingApiKey.slice(-4);
    apiKeyDisplay = ` [${masked}]`;
  }

  const rawApiKey = await prompt(
    `  ? Anthropic API key (sk-ant-...)${apiKeyDisplay}: `,
  );
  const apiKey = rawApiKey || existingApiKey;

  if (apiKey && !apiKey.startsWith("sk-ant-") && !apiKey.startsWith("sk-")) {
    console.log(
      "  ⚠️  Key format looks unexpected (expected sk-ant-... or sk-...). Proceeding anyway.",
    );
  }
  console.log("");
  ...
  console.log(
    "  Trestle can automatically downgrade models when you approach the cap.",
  );
  console.log("");

  const defaultDaily = existingDailyUsd ?? 10;
  const rawDaily = await prompt(
    `  ? Daily budget cap in USD [default: $${defaultDaily.toFixed(2)}]: `,
  );
  ...
  if (rawDaily) {
    ...
    if (!isNaN(parsed) && parsed > 0) {
      ...
      console.log(
        `  ⚠️  Invalid value, using default $${defaultDaily.toFixed(2)}`,
      );
    }
  }
  console.log("");
  ...
  console.log(hr);
  console.log("  How should Trestle handle budget breaches?");
  console.log("");
  console.log(
    "    [1] Smart  — Auto-downgrade to cheaper models (recommended)",
  );
  console.log(
    "    [2] Strict — Block all requests when daily limit is reached (402)",
  );
  ...
  // Ensure base Trestle config exists (device_id, telemetry, etc.)
  ...
  if (apiKey) {
    ...
    const accounts =
      (anthropic["accounts"] as Array<Record<string, unknown>>) ?? [];
  ...
  console.log("");
  console.log("  ✅ Trestle configured!");
  console.log("");
  console.log("  Next steps:");
  console.log("    1. Start the proxy:");
  console.log("         trestle start");
  console.log("");
...
function handleBudgetCommand(args: string[]): void {
  ...
  if (sub === "status") {
    ...
    if (cap !== null) {
      ...
      const bar =
        pct >= 100 ? "🚫 BLOCKED" : pct >= 80 ? "⚠️  warning" : "✅ ok";
      console.log(
        `   Daily cap:   $${todaySpend.toFixed(4)} / $${cap.toFixed(2)} (${pct.toFixed(1)}%) ${bar}`,
      );
    } else {
      console.log(`   Daily cap:   $${todaySpend.toFixed(4)} / unlimited`);
    }
    console.log(
      `   Daily:       $${status.dailySpend.toFixed(4)} / $${status.dailyLimit} (${status.dailyPercent.toFixed(1)}%)`,
    );
    console.log(
      `   Hourly:      $${status.hourlySpend.toFixed(4)} / $${status.hourlyLimit} (${status.hourlyPercent.toFixed(1)}%)`,
    );
  ...
  if (sub === "history") {
    ...
    if (history.length === 0) {
      ...
      for (const day of history) {
        const pct =
          cap !== null && cap > 0
            ? ` (${((day.totalSpend / cap) * 100).toFixed(1)}%)`
            : "";
...
function handleCacheCommand(args: string[]): void {
  ...
  if (sub === "stats") {
    ...
    console.log(
      `   Hits: ${stats.hits}  Misses: ${stats.misses}  Bypasses: ${stats.bypasses}`,
    );
    console.log(
      `   Saved:          $${stats.savedCostUsd.toFixed(4)} across ${stats.savedRequests} requests`,
    );
    console.log("");
    ...
    if (models.length > 0) {
      ...
      for (const [model, m] of models) {
        console.log(
          `     ${model}: ${m.entries} entries, ${m.hits} hits, $${m.savedCostUsd.toFixed(4)} saved`,
        );
      }
    }
    ...
    if (tasks.length > 0) {
      ...
      for (const [task, t] of tasks) {
        console.log(
          `     ${task}: ${t.entries} entries, ${t.hits} hits, $${t.savedCostUsd.toFixed(4)} saved`,
        );
      }
    }
    console.log("");
...
function handleAgentsCommand(subArgs: string[]): void {
  ...
  const registryFile = join(homedir(), ".trestle", "agents.json");
...
async function handlePolicyCommand(subArgs: string[]): Promise<void> {
  ...
  if (sub === "show") {
    if (!existsSync(POLICY_FILE)) {
      console.log(
        "No policy file found. Run 'trestle policy init' to create one.",
      );
      return;
    }
  ...
  if (sub === "init") {
    ...
    for (const [fp, entry] of Object.entries(registry)) {
      const avgCost =
        entry.totalRequests > 0 ? entry.totalCost / entry.totalRequests : 0;
      const preferred =
        avgCost > 0.01 ? "anthropic/claude-sonnet-4-6" : "cheapest-capable";
      agents[entry.name] = {
        fingerprint: fp,
        preferred,
      };
    ...
    mkdirSync(join(homedir(), ".trestle"), { recursive: true });
    writeFileSync(POLICY_FILE, yaml, "utf-8");
    const count = Object.keys(agents).length;
    console.log(
      `Wrote ${POLICY_FILE} with ${count} agent${count !== 1 ? "s" : ""}. Edit to customize, then restart the proxy.`,
    );
    return;
  }
  ...
  if (sub === "set-agent") {
    ...
    if (!name) {
      console.error(
        "Usage: trestle policy set-agent <name> --preferred <model> [--escalate <model>] [--budget <amount>]",
      );
      process.exit(1);
    }
    ...
    if (existsSync(POLICY_FILE)) {
      try {
        rawPolicy = (yamlLoad(readFileSync(POLICY_FILE, "utf-8")) as Record<
          string,
          unknown
        >) ?? { version: 1 };
      } catch {
    ...
    const agentsObj = rawPolicy.agents as Record<
      string,
      Record<string, unknown>
    >;
    ...
    console.log(
      `Updated policy for agent "${name}": preferred=${preferred}${escalateTo ? ` escalateTo=${escalateTo}` : ""}${budget !== undefined ? ` budgetPerDay=$${budget}` : ""}`,
    );
    return;
  }
  ...
  if (sub === "auto") {
    ...
    if (analyses.length === 0) {
      ...
      console.log("  Run Trestle for at least a day and try again.");
  ...
  if (sub === "test") {
    ...
    if (entries.length === 0) {
      console.log(
        "No routing log entries found. Run Trestle first to generate traffic.",
      );
      return;
    }
    ...
    console.log(
      `Replaying last ${entries.length} requests against ${policySource}...`,
    );
    console.log("");
    ...
    if (changed === 0) {
      console.log(
        `  All ${total} requests would route identically. No savings from this policy.`,
      );
    } else {
    ...
    const totalSavings = Math.max(
      0,
      changes.reduce((sum, c) => sum + c.savedCost, 0),
    );
    console.log("");
    console.log(
      `Estimated savings across these ${total} requests: $${totalSavings.toFixed(4)}`,
    );
  ...
  if (sub === "rename") {
    ...
    if (!arg || !newName) {
      console.error(
        "Usage: trestle policy rename <fingerprint-or-name> <new-name>",
      );
      process.exit(1);
    }
    ...
    if (existsSync(POLICY_FILE)) {
      try {
        let rawPolicy = yamlLoad(readFileSync(POLICY_FILE, "utf-8")) as Record<
          string,
          unknown
        >;
        if (!rawPolicy) rawPolicy = { version: 1 };
        const agents = rawPolicy["agents"] as
          | Record<string, unknown>
          | undefined;
  ...
  if (sub === "reset") {
    ...
    if (!confirm) {
      ...
      console.log(
        "This will remove your routing policy and return to passthrough mode.",
      );
      console.log(
        `All ${agentCount} agent rule${agentCount !== 1 ? "s" : ""} will be deleted.`,
      );
      console.log(
        "Run again with --confirm to proceed: trestle policy reset --confirm",
      );
      return;
    }
    ...
    console.log("Policy reset. Trestle is now in passthrough mode.");
    console.log(`Backup saved at: ${POLICY_FILE}.bak`);
    return;
  }

  console.log(
    "Usage: trestle policy [auto|suggest|test|rename|reset|init|show|set-agent]",
  );
}
...
function _buildPolicyYaml(
  analyses: AgentAnalysis[],
  suggestions: PolicySuggestion[],
): string {
  const isoDate = new Date().toISOString().slice(0, 10);
  const header = [
    "# Trestle routing policy",
    `# Generated by \`trestle policy auto\` on ${isoDate}`,
    "# Edit manually or re-run `trestle policy auto` to regenerate.",
    "# Reset to passthrough: `trestle policy reset`",
    "",
  ].join("\n");
...
async function _runPolicyAutoDisplay(
  ...
  for (const a of analyses) {
    ...
    console.log(
      `  ${fpShort}  →  "${namePad}"  (${pct}% ${a.dominantTask} tasks, avg ${avgK}K tokens${estFlag}, $${cost}/day)`,
    );
  ...
  if (lowDataAgents.length > 0) {
    ...
    for (const name of lowDataAgents) {
      ...
      console.log(
        `  ⚠ ${name}: only ${a.totalRequests} requests — suggestions may be inaccurate`,
      );
    }
  }
  ...
  console.log("");
  console.log(
    "Suggested policy (based on task patterns + your available keys):",
  );
  console.log("");
  ...
  for (const s of suggestions) {
    ...
    if (s.noSuggestion) {
      ...
      const escNote = s.escalateTo
        ? `  (escalate to ${s.escalateTo} on complexity)`
        : "";
      const savingsNote =
        s.estimatedDailySavings > 0.01
          ? `saves ~$${s.estimatedDailySavings.toFixed(2)}/day`
          : "(no change)";
      console.log(
        `  ${namePad}  →  ${s.suggestedModel}${escNote}  ${savingsNote}`,
      );
  ...
  console.log("");
  console.log(
    `Projected monthly savings: ~$${Math.round(totalMonthlySavings)}`,
  );
  console.log("");

  if (!interactive) {
    // policy suggest — print CTA and exit
    console.log("To apply this policy, run: trestle policy auto");
    return;
  }
  ...
  if (existsSync(POLICY_FILE)) {
    ...
    console.log(
      `⚠ Existing policy will be replaced. Previous: ${existingAgents || "no agents"}.`,
    );
    console.log("");
  }
  ...
  writeFileSync(tmp, yaml, "utf-8");
  renameSync(tmp, POLICY_FILE);
  const agentCount = analyses.filter(
    (_, i) => !suggestions[i]?.noSuggestion,
  ).length;
  console.log("");
  console.log(
    `Policy applied. Routing active for ${agentCount} agent${agentCount !== 1 ? "s" : ""}.`,
  );
  console.log("Run `trestle policy show` to review.");
}
...
async function handleSetupCommand(): Promise<void> {
  ...
  console.log("Welcome to Trestle.");
  ...
  const allProviders: Array<{
  ...
  }> = [
    {
      ...
      detected: detectedProviders.includes("anthropic"),
      detectedEnv: process.env["ANTHROPIC_API_KEY"]
        ? "ANTHROPIC_API_KEY"
        : undefined,
    },
    {
      name: "openai",
      displayName: "OpenAI",
      detected: detectedProviders.includes("openai"),
      detectedEnv: process.env["OPENAI_API_KEY"] ? "OPENAI_API_KEY" : undefined,
    },
    {
      name: "google",
      displayName: "Google Gemini",
      detected: detectedProviders.includes("google"),
      detectedEnv: process.env["GEMINI_API_KEY"]
        ? "GEMINI_API_KEY"
    ...
    },
    {
      name: "groq",
      displayName: "Groq",
      detected: detectedProviders.includes("groq"),
      detectedEnv: process.env["GROQ_API_KEY"] ? "GROQ_API_KEY" : undefined,
    },
    {
      name: "openrouter",
      displayName: "OpenRouter",
      detected: detectedProviders.includes("openrouter"),
      detectedEnv: process.env["OPENROUTER_API_KEY"]
        ? "OPENROUTER_API_KEY"
        : undefined,
    },
  ...
  for (const p of allProviders) {
    ...
    const detectedNote =
      p.detected && p.detectedEnv ? ` (detected: ${p.detectedEnv})` : "";
  ...
  if (!isTTY) {
    ...
    if (confirmedProviders.length === 0) {
      console.log(
        "  ⚠ No provider keys detected. Trestle will run in passthrough/observe mode.",
      );
    ...
    console.log("");
    console.log("  Trestle is ready.");
    console.log("");
    console.log("  Proxy:        http://localhost:4100");
    console.log("  Configure:    ANTHROPIC_BASE_URL=http://localhost:4100");
    console.log("");
    console.log("  Next: run Trestle for a day, then:");
    console.log("    trestle policy auto    # auto-configure routing");
    console.log("    trestle policy suggest  # preview recommendations");
    console.log("");
  ...
  const addInput = await prompt(
    "Press Enter to confirm, or list provider names to add (comma-separated): ",
  );
  if (addInput) {
    const extras = addInput
      .split(",")
  ...
  console.log("");
  console.log("Step 2/3 — Routing mode");
  console.log("  How should Trestle route your requests?");
  console.log("");
  console.log(
    "  1. Auto (recommended) — route by task complexity, cheapest capable model",
  );
  ...
  if (routingMode === "auto") {
    if (existsSync(POLICY_FILE)) {
      console.log(
        "  (Existing policy preserved — run `trestle policy auto` to update it)",
      );
    } else {
      const autoYaml = [
        "# Trestle routing policy",
        "# Run `trestle policy auto` after a day of traffic to auto-configure.",
        "# Auto mode: suggestions will be based on your actual traffic patterns.",
        "",
        "version: 1",
        "",
        "# agents will be added here by `trestle policy auto`",
        "",
      ].join("\n");
      writeFileSync(POLICY_FILE, autoYaml, "utf-8");
    }
  } else if (routingMode === "manual") {
    console.log("  Run `trestle policy init` to scaffold a policy template.");
  }
