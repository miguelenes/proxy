...
vi.mock("../src/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    getConfigDir: () =>
      (global as any).__TEST_CONFIG_DIR__ || actual.getConfigDir(),
...
describe("star-nudge: disabled in local-only Trestle", () => {
...
describe("star-nudge: output format", () => {
  it("nudge contains the ⭐ emoji and the GitHub URL", async () => {
    ...
    expect(output).toContain("https://github.com/Trestle/proxy");
  });
  ...
  it("message does not contain the request count (unlike signup nudge)", async () => {
    ...
    const output = stderrSpy.mock.calls[0][0] as string;
    expect(output).toContain("Enjoying Trestle");
...
describe("star-nudge: uses separate flag file from signup nudge", () => {
  it("star-nudge-shown.json is separate from nudge-shown.json", async () => {
    ...
    expect(fs.existsSync(path.join(tmpDir, "star-nudge-shown.json"))).toBe(
      true,
    );
...
describe("star-nudge: threshold constant", () => {
  ...
  it("GITHUB_URL points to Trestle/proxy", async () => {
    const { GITHUB_URL } = await import("../src/star-nudge.js");
    expect(GITHUB_URL).toBe("https://github.com/Trestle/proxy");
  });
});
